...
namespace WebServiceStudio
{
  ...
  internal class Wsdl
  {
    ...
    public X509Certificate GetClientCertificate(string certname, StoreLocation storeLocation)
    {
      X509Store store = new X509Store(StoreName.My, storeLocation);
      store.Open(OpenFlags.ReadOnly);
      X509CertificateCollection cert = store.Certificates.Find(X509FindType.FindBySubjectName, certname, true);
      return cert[0];
    }
    ...
    {
      ICodeCompiler compiler = this.codeProvider.CreateCompiler();
      string location = "";
      if ((this.WsdlProperties.ProxyBaseType != null) && (this.WsdlProperties.ProxyBaseType.Length > 0))
      {
